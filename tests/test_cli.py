...
def test_cli_run_wires_seams_and_tears_down(monkeypatch, capsys):
    ...
    monkeypatch.setattr(cli, "make_bench_fn", lambda sweep: (lambda url: Metrics(50, 10, 500, 1000, 1.0, 8, 16)))
...
def test_cli_tears_down_even_on_error(monkeypatch):
    ...
    monkeypatch.setattr(cli, "make_deploy_fn", lambda: boom)
    monkeypatch.setattr(cli, "make_bench_fn", lambda sweep: (lambda url: None))
